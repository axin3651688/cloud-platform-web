<template>
  <div class="wrapper">
    <a-row :gutter="16">
      <a-col :md="6"
             :sm="24">
        <left-tree :treeData="comTreeData"
                   @select="onCompanySelect"></left-tree>
        <a-button type="primary"
                  @click="saveCompany"
                  v-action:addCompany>添加公司</a-button>
      </a-col>
      <a-col :md="18"
             :sm="24">
        <div v-show="!onAdd">
          <system-collapse :title="'企业形象'"
                           :subtitle="'当前企业的标志。'">
            <system-upload v-show="showBtn"
                           :showDel="true"
          ...
          <system-collapse :title="'企业资料'"
                           :subtitle="'当前企业名称所属地区。'">
            <company-info-list :nodes="curCorporateInformation()"></company-info-list>
            <a-col :md="2"
                   :sm="6">
              <a-button type="primary"
                        @click="onEditCorporateInformation"
                        v-show="showBtn"
                        v-action:editCompany>修改</a-button>
            </a-col>
          </system-collapse>
          <system-collapse :title="'企业属性'"
                           :subtitle="'当前规模和所属行业。'">
            <company-info-list :nodes="curEnterpriseAttribute()"></company-info-list>
            <a-col :md="2"
                   :sm="6">
              <a-button type="primary"
                        @click="onEditEnterpriseAttribute"
                        v-show="showBtn"
                        v-action:editCompany>修改</a-button>
            </a-col>
          </system-collapse>
          <system-collapse :title="'企业级别'"
                           :subtitle="'当前企业在集团公司下的级别和排序。'">
            <company-info-list :nodes="curEnterpriseLevel()"></company-info-list>
            <a-col :md="2"
                   :sm="6">
              <a-button type="primary"
                        @click="onEnterpriseLevel"
                        v-show="showBtn"
                        v-action:editCompany>修改</a-button>
            </a-col>
          </system-collapse>
          <system-collapse :title="'企业其他资料'"
                           :subtitle="'录入企业时新添加的字段属性。'">
            <company-info-list :nodes="curEnterpriseOther()"></company-info-list>
            <a-col :md="2"
                   :sm="6">
              <a-button type="primary"
                        @click="onEnterpriseOther"
                        v-show="showBtn"
                        v-action:editCompany>修改</a-button>
            </a-col>
          </system-collapse>
          <system-collapse v-if="$auth('delCompany')"
                           v-show="!selectGroupCompany"
                           :title="'刪除公司'"
                           :subtitle="'可以选择刪除当前企业,刪除后企业所有数据都将刪除,并且无法撤销,请谨慎操作。'">
            <company-info-list></company-info-list>
            <a-col :md="2"
                   :sm="6">
              <a-button type="primary"
                        @click="onDelete"
                        v-show="showBtn">删除</a-button>
            </a-col>
          </system-collapse>
          <system-collapse v-if="$auth('delCompany')"
                           v-show="selectGroupCompany"
                           :title="'转让公司'"
                           :subtitle="'可以选择转让当前企业,无法撤销,请谨慎操作。'">
            <company-info-list></company-info-list>
            <a-col :md="2"
                   :sm="6">
              <a-button type="primary"
                        v-show="showBtn">转让</a-button>
            </a-col>
          </system-collapse>
        </div>
        <company-save-form v-show="onAdd"
                           :pid="curSelectComId"
                           @onBack="onFormClose"></company-save-form>
      </a-col>
    </a-row>
    <corporate-information-modal ref="corporateInformationModal"
                                 @onBack="onModalClose"></corporate-information-modal>
    <enterprise-attribute-modal ref="enterpriseAttributeModal"
                                @onBack="onModalClose"></enterprise-attribute-modal>
    <enterprise-level-modal ref="enterpriseLevelModal"
                            @onBack="onModalClose"></enterprise-level-modal>
    <enterprise-other-modal ref="enterpriseOtherModal"
                            @onBack="onModalClose"></enterprise-other-modal>
  </div>
</template>
